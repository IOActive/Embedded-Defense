...
factory = RequestMock()
request = factory.get('http://localhost:8000/admin/login/?next=/admin/')
request1 = factory.get('http://localhost:8000/admin/login/?next=/admin/acunetix/')
request2 = factory.get('http://localhost:8000/admin/login/?next=/admin/acunetix/x.bacKup')
request_method = "FAKE"
...
handeling.checkHostname(request1, 'www.example.com')


# test nonExistingFile takes only request as a parameter
handeling.nonExistingFile(request2)
...
# check fake input
request3 = factory.post('http://localhost:8000/', {"input_name": "i am value"})
handeling.checkFakeInput(request3, 'input_name', 'i am valu')


# making 10 request to check speed of requess per time
